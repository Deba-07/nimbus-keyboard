...
const ColorChanger: FC<ColorChangerProps> = ({ heading, description }) => {
    ...
    >
      ...
      >
        <text
          ...
          x="50%"
          y="50%"
          className="font-black-slanted fill-white/20 uppercase"
        >
      ...
      {/* content */}
      <div className="relative mx-auto flex w-full max-w-7xl flex-col gap-8 px-6 pt-10 pb-16 lg:flex-row">
        <div className="max-w-md">
          <h2 className="font-bold-slanted mb-2 text-4xl uppercase lg:text-6xl">
            {heading}
          </h2>

          <p className="leading-relaxed text-white/80 lg:text-lg">
            {description}
          </p>
        </div>
          ...
          {KEYCAP_TEXTURES.map((texture) => (
            <li key={texture.id}>
              <button
                ...
                className={clsx(
                  "flex aspect-square flex-col items-center justify-center rounded-lg border-2 p-4 transition-all duration-300",
